		...
		<div className="sms-settings-tab">
			...
			>
				...
				<FormGroup
					...
					description={__('نام ارائه‌دهنده سرویس پیامک', 'tabesh-v2')}
				>
					<select
						name="provider"
					...
					>
						<option value="kavenegar">{__('کاوه‌نگار', 'tabesh-v2')}</option>
						<option value="farazsms">{__('فراز اس‌ام‌اس', 'tabesh-v2')}</option>
						<option value="smsir">{__('اس‌ام‌اس.آی‌آر', 'tabesh-v2')}</option>
						<option value="melipayamak">{__('ملی پیامک', 'tabesh-v2')}</option>
						<option value="custom">{__('سفارشی', 'tabesh-v2')}</option>
					</select>
				</FormGroup>

				<FormGroup
					label={__('API Key', 'tabesh-v2')}
					description={__('کلید API دریافتی از سرویس پیامک', 'tabesh-v2')}
				>
					<TextInput
						name="api_key"
				...
				<FormGroup
					label={__('URL وب‌سرویس (برای سرویس سفارشی)', 'tabesh-v2')}
					description={__('آدرس API در صورت استفاده از سرویس سفارشی', 'tabesh-v2')}
				>
					<TextInput
						name="api_url"
			...
			<Section
				...
				description={__('قالب پیامک‌های ارسالی به مشتریان', 'tabesh-v2')}
			>
				<FormGroup>
					<Checkbox
						...
						onChange={handleChange}
						label={__('ارسال پیامک هنگام ثبت سفارش', 'tabesh-v2')}
					/>
				</FormGroup>

				<FormGroup
					label={__('قالب پیامک ثبت سفارش', 'tabesh-v2')}
					description={__('متغیرهای قابل استفاده: {customer_name}, {order_number}, {total_amount}', 'tabesh-v2')}
				>
					<TextArea
						...
						onChange={handleChange}
						rows={4}
						placeholder={__('سلام {customer_name}، سفارش شما با شماره {order_number} ثبت شد.', 'tabesh-v2')}
					/>
				</FormGroup>
				...
				<FormGroup>
					<Checkbox
						...
						onChange={handleChange}
						label={__('ارسال پیامک هنگام تغییر وضعیت', 'tabesh-v2')}
					/>
				</FormGroup>

				<FormGroup
					label={__('قالب پیامک تغییر وضعیت', 'tabesh-v2')}
					description={__('متغیرهای قابل استفاده: {customer_name}, {order_number}, {status}', 'tabesh-v2')}
				>
					<TextArea
						...
						onChange={handleChange}
						rows={4}
						placeholder={__('سفارش {order_number} به وضعیت {status} تغییر یافت.', 'tabesh-v2')}
					/>
				</FormGroup>
				...
				<FormGroup>
					<Checkbox
						...
						onChange={handleChange}
						label={__('ارسال پیامک هنگام آماده شدن برای ارسال', 'tabesh-v2')}
					/>
				</FormGroup>

				<FormGroup
					label={__('قالب پیامک آماده ارسال', 'tabesh-v2')}
					description={__('پیامک اطلاع‌رسانی آماده بودن سفارش برای ارسال', 'tabesh-v2')}
				>
					<TextArea
						...
						onChange={handleChange}
						rows={4}
						placeholder={__('سفارش شما آماده ارسال است.', 'tabesh-v2')}
					/>
				</FormGroup>
			</Section>
			...
			>
				...
				<FormGroup
					...
					description={__('تعداد دفعات تلاش مجدد در صورت خطا', 'tabesh-v2')}
				>
					<TextInput
						name="max_retries"
