 ...
 */
export const TextInput = ({ name, value, onChange, placeholder = '', type = 'text' }) => {
	return (
		<input
			type={type}
			name={name}
 ...
 * Textarea Component.
 */
export const TextArea = ({ name, value, onChange, placeholder = '', rows = 4 }) => {
	return (
		<textarea
			name={name}
			value={value || ''}
 ...
 * Select Component.
 */
export const Select = ({ name, value, onChange, options, placeholder = '' }) => {
	return (
		<select name={name} value={value || ''} onChange={onChange} className="regular-text">
